    ...
    def train(self, num_steps):
        ...
        for cur_step in tt:
            ...
            if (init_global_step+cur_step%self.config.num_reset_steps)< self.config.num_burnin_steps:
                _, _, walk_loss, error, acpt_rate = self.sess.run([
                    global_step_inc_op, walk_op, walk_loss_node, error_node, acpt_rate_node])

                walk_loss_per_epoch.update(walk_loss)
                error_per_epoch.update(error)
                acpt_rate_per_epoch.update(acpt_rate)

                summaries_dict = {'train/walk_loss': walk_loss_per_epoch.avg,
                                  'train/error': error_per_epoch.avg,
                                  'train/acpt_rate': acpt_rate_per_epoch.avg}
            else:
                _ = self.sess.run([
                ...
                summaries_dict = {'train/walk_loss': walk_loss_per_epoch.avg,
                                  'train/fisher_loss': fisher_loss_per_epoch.avg,
                                  'train/error': error_per_epoch.avg,
                                  'train/acpt_rate': acpt_rate_per_epoch.avg}
