...
class MovingAverageMeter:
    ...
    def update(self, val):
        if len(self.vals) == self.interval:
            popped_val = self.vals.pop(0)
            self.vals.append(val)
            self.avg = self.avg+(val-popped_val)/self.interval
        elif len(self.vals) > self.interval:
            raise Error
        else:
            self.vals.append(val)
            self.avg = ((len(self.vals)-1)*self.avg+val)/len(self.vals)
