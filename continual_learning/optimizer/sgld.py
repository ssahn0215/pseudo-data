...
class EasyStochasticGradientLangevinDynamics(tf.train.Optimizer):
    def __init__(self, learning_rate=0.001, data_size=1, use_locking=False, name="SGLD"):
        super(EasyStochasticGradientLangevinDynamics, self).__init__(use_locking, name)
    ...
    def _apply_dense(self, grad, var):
        ...
        num_pseudo_batchs = tf.cast(self._data_size, grad.dtype)
        noise = tf.random_normal(
            tf.shape(grad), stddev=tf.sqrt(lr_t), dtype=grad.dtype)
        new_grad = 0.5*lr_t*grad+noise
        var_update = state_ops.assign_sub(var, new_grad)
...
class StochasticGradientLangevinDynamics(tf.train.Optimizer):
  ...
  `1/data_size`, where data_size is the number of minibatches
  in the data.  I.e., it should be divided by the `data_size` term
  described below.
  Args:
  ...
  def _apply_noisy_update(self, mom, grad):
    ...
    return (
        preconditioner*grad
        +tf.random_normal(tf.shape(grad), mean=0., stddev=1.0, dtype=grad.dtype)
        *stddev*tf.sqrt(2*preconditioner/tf.cast(self._data_size, grad.dtype)))
